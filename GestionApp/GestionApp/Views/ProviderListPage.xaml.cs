...
namespace GestionApp.Views
{
    ...
    public partial class ProviderListPage : ContentPage
    {
        ...
        async void OnDeleteProviderButtonClicked(object sender, EventArgs e)
        {
            var button = sender as Button;
            var provider = button?.CommandParameter as Provider;
            if (provider != null)
            {
                await App.Database.DeleteProviderAsync(provider);
                LoadProviders();
            }
        }
    }
